...
function StatCell({ stat }: { stat: (typeof stats)[number] }) {
  ...
  return (
    <motion.div
      ...
      className="group flex cursor-default flex-col gap-8 bg-navy p-8 transition-colors duration-500 hover:bg-navy-600 md:p-10"
    >
...
export function TrackRecordStats() {
  return (
    <section id="track-record" className="mx-auto max-w-[1440px] px-4 py-6 md:px-3">
      <div className="overflow-hidden rounded-card bg-navy px-8 py-16 text-white md:px-16 md:py-20">
        ...
        <motion.div className="mt-10 grid gap-px overflow-hidden rounded-2xl bg-white/12 sm:grid-cols-2 lg:grid-cols-4" variants={stagger(0.1)} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
          {stats.map((s) => (
